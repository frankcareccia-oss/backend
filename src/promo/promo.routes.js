    ...
    try {
      ...
      const promotion = await prisma.promotion.create({
        data: {
          ...
          ...(promotionType === "tiered" && Array.isArray(tiers) && tiers.length > 0 ? {
            tiers: {
              create: tiers.map((t, idx) => ({
                tierName: String(t.tierName || `Tier ${idx + 1}`).trim(),
                tierLevel: t.tierLevel || idx + 1,
                threshold: parseInt(t.threshold, 10),
                rewardType: t.rewardType || "discount_fixed",
                rewardValue: t.rewardValue != null ? parseInt(t.rewardValue, 10) : null,
                rewardNote: t.rewardNote || null,
              })),
            },
          } : {}),
          ...
          ...(promotionType === "bundle" && bundleDefinition ? {
            bundleDefinition: {
              create: {
                items: bundleDefinition.items || [],
                bundlePriceCents: parseInt(bundleDefinition.bundlePriceCents, 10) || 0,
                savingsCents: parseInt(bundleDefinition.savingsCents, 10) || 0,
                validityDays: parseInt(bundleDefinition.validityDays, 10) || 30,
              },
            },
          } : {}),
        },
  ...
  async (req, res) => {
    try {
      ...
      const clone = await prisma.promotion.create({
        ...
        include: { items: { include: { promoItem: { include: { skus: true } } } }, category: true, tiers: { orderBy: { tierLevel: "asc" } }, conditions: true, bundleDefinition: true },
      });
